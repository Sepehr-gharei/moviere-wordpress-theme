...
// Add second thumbnail meta box
function add_second_thumbnail_meta_box() {
    add_meta_box(
        'second-thumbnail',
        'تصویر دوم (تانمای دوم)',
        'render_second_thumbnail_meta_box',
        'post',
        'side',
        'low'
    );
}
add_action('add_meta_boxes', 'add_second_thumbnail_meta_box');

// Render meta box content
function render_second_thumbnail_meta_box($post) {
    wp_nonce_field('second_thumbnail_nonce', 'second_thumbnail_nonce');
    $thumbnail_id = get_post_meta($post->ID, '_second_thumbnail_id', true);
    echo '
    <div class="second-thumbnail-container">
        <button id="upload_second_thumbnail" class="button">آپلود یا انتخاب تصویر</button>
        <input type="hidden" name="_second_thumbnail_id" id="_second_thumbnail_id" value="' . esc_attr($thumbnail_id) . '">
        <div id="second_thumbnail_preview">' . 
            ($thumbnail_id ? wp_get_attachment_image($thumbnail_id, 'thumbnail') : '') . 
        '</div>
        <button id="remove_second_thumbnail" class="button" style="' . ($thumbnail_id ? '' : 'display:none;') . '">حذف تصویر</button>
    </div>';
}

// Save meta data
function save_second_thumbnail_meta($post_id) {
    if (!isset($_POST['second_thumbnail_nonce']) || 
        !wp_verify_nonce($_POST['second_thumbnail_nonce'], 'second_thumbnail_nonce')) {
        return;
    }
    
    if (defined('DOING_AUTOSAVE') && DOING_AUTOSAVE) return;
    
    if (isset($_POST['_second_thumbnail_id'])) {
        update_post_meta(
            $post_id,
            '_second_thumbnail_id',
            sanitize_text_field($_POST['_second_thumbnail_id'])
        );
    }
}
add_action('save_post', 'save_second_thumbnail_meta');
